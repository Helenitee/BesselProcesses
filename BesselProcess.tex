...
\int_{\R_+} \varphi(y) L_t^y(Y) dy &= \int_{\R_+} \tilde\varphi(x) L_t^x(X) dx\\
                       ...
                       &\overset{y = f(x)}{=} \int_{\R_+}\varphi(y) f'( (f^{-1}(y))^2) L_t^{f^{-1}(y)}d(f^{-1}(y)) \\
  &=  \int_{\R_+}\varphi(y) f'( (f^{-1}(y))^2) L_t^{f^{-1}(y)} \;\frac{1}{f'(f^{-1}(y))} dy.
\end{alignat*}
...
%\paragraph{$\delta \in (0,1]$}
%\paragraph{$\delta \in (1,2)$}
\end{proof}
\section{Cas $\delta = 2 $}
\section{Cas $\delta > 2 $}
