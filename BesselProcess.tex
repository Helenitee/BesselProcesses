...
% motivation
Ce travail a été motivé principalement par l'équation différentielle stochastique (EDS) de Keller Segel, qui modélise un système de particules stochastiques en interaction singulière.
% applications
Une des applications les plus étudiées, est le système de chimiotaxie\footnote{La chimiotaxie est le phénomène par lequel des cellules ou des particules orientent leur déplacement en réponse à la présence de certaines substances chimiques dans leur environnement qui souvant est émise par ces mêmes cellules. \cite[wikiChimioaxie].} de la bactérie \textit{Escherichia coli}. Celle-ci se déplace grâce à des flagelles\footnote{Les flagelles sont de longs appendices filiformes, constitués principalement de protéines, situés à la surface de certaines cellules. Ils assurent la motilité cellulaire.} attachés à leur corps. L'étude de ce phénomène commence en 1953 dans des travaux de C.S. Patlak. En 1970, Evelyn Fox Keller et Lee A. Segel présentent un système de quatres équations différentielles paraboliques qui décrivent ces mouvements.\\


L'idée de l'équation de Keller-Segel est de décrire la position en temps $t\in \R^+$ d'une particule en interaction avec $N-1$ particules. Ces particules exercent les unes sur les autres une force d’attraction d’intensité variable. On appelle \textit{diffusion} le terme qui décrit les mouvements ``aléatoires'' d'une particule et \textit{drift} le terme qui décrit les mouvements liés à l'attraction d'une particule par rapport aux autres. Si l'intensité d'attraction est faible, alors il existe une solution globale de l'équation, tandis que si l'intensité d'attraction est forte, la solution explose puisque on trouve une singularité aux points de collision. 

% problématique
Une des questions qu'on peut se poser est, comment le terme du \textit{drift} se comporte-t-il lorsque l’on considère une grande population de particules, c'est-à-dire, existe-t-il une limite lorsque $N \to \infty$ ? Plus précisément, peut-on définir un \textit{drift} lorsqu'on visite une infinité de fois les points de singularité (i.e. de collisions), et comment influence-t-il le comportement global du système ?

% lien avec les Bessel
...
\begin{proof} Soit $f \in C^2$ tel que $f'' \in \mathcal C_c$. Posont $\varphi = f''$. %En appliquant la formule d'Itō au mouvement brownien $(W_t)_{t\geq0}$ on remarque que les deux premiers termes du RHS des deux formues sont les mêmes. Ainsi, il suffit de montrer que p.s.
%  $$\int_0^t \varphi(W_s) \d s = \int_\R \varphi(a) L_t^a \d a.$$
Remarquons que :
  \begin{align*}
...
$$\lim_{n \to \infty} \E(|\psi(z_n) - \psi(z)|^2) \leq \E\left(\int_0^t\lim_{n \to \infty}\left( \varphi(z_n)\1_{W_s > z_n} - \varphi(z)\1_{W_s >z}\right)^2 \d s \right) = 0$$
  ...
  En fixant $n$ on a \text{\hyperlink{termI}{$I$}}$=$\text{\hyperlink{R_n}{$R_n(\varphi)$}}. Par souci de notation, posons $b = -m + 2m \; \frac{k}{n}$. Par linéarité de l'intégrale, on peut dire que :
\begin{align*}
  ...
  Enfin, il suffit de voir qu'en intégrant \eqref{eq:f'} de $0$ à $t$  par rapport à $W_s$ on retrouve \text{\hyperlink{I}{$I$}}. Pour cela, on regarde le crochet de la différence des deux termes au carré.
  \begin{align*}
...
%Par la formule d'occupation \autoref{thm:occupationBessel}, l'intégrale \hyperlink{term3}{$J_3^\varepsilon$} est égale à :
%\begin{alignat*}{2}
...
%  &= \dfrac{1}{2(2 - \delta)}\int_{\R^+} \ell_t^a(\rho) \left(\dfrac{\delta\;a^{\delta - 1}}{(a^2 + \varepsilon)^{1/2}} - \dfrac{a^{\delta + 1}}{(a^2 + \varepsilon)^{3/2}}\right)\d a,
...
%  &= \dfrac{1}{2(2 - \delta)}\int_{\alpha}^\beta \ell_t^a(\rho) \left(\dfrac{\delta\;a^{\delta - 1}}{(a^2 + \varepsilon)^{1/2}} - \dfrac{a^{\delta + 1}}{(a^2 + \varepsilon)^{3/2}}\right)\d a,\\
%  &\leq \dfrac{1}{2(2 - \delta)} ||\ell_t(\rho)||_\infty (\beta - \alpha)\;\int_{\alpha}^\beta \left(\dfrac{\delta\;a^{\delta - 1}}{(a^2 + \varepsilon)^{1/2}} - \dfrac{a^{\delta + 1}}{(a^2 + \varepsilon)^{3/2}}\right)\d a,
...
%   \text{\hyperlink{term3}{$J_3^\varepsilon$}} &\leq \dfrac{1}{2(2 - \delta)} ||\ell_t(\rho)||_\infty (\beta - \alpha)\; \left[\dfrac{a^\delta}{(a^2 + \varepsilon)^{1/2}}\right]_\alpha^\beta < \infty.
%\end{alignat*}
%
...
%{\color{red}
%  \begin{remark}
%    Ce qui nous interesse est de faire sens de l'EDS qui définit les processus de Bessel pour $\delta \in (0,1)$. En effet, ce qui est curieux de l'équation \eqref{eq:EDSBessel3} est qu'on intègre sur $\R_+$ une fonction qui multiplie $a^{\delta -2}$. Or, $\delta -2 < -1$ et donc n'est pas intégrable. De plus $\ell_t^a$ est continue à support compact et $\ell_t^0$ est une constante. On pourrait donc se dire que $a^{\delta -2}(\ell_t^a - \ell_t^0)$ n'est donc pas intégrable mais en fait le terme $\ell_t^a - \ell_t^0$ rend le produit intégrable. Pour $\delta \geq 1$ on n'a pas de singularité en zéro. En effet, le drift, c.à.d le terme $\frac{\delta-1}{2 \; \rho_s}$, disparait en $\delta = 1$ et est strictement positif pour $\delta>1$. En prenant l'équation \eqref{eq:EDSBessel3} et on la dérivant on obtient $\d \rho_t = \frac{\delta-1}{2\;\rho_t}\d t  + \d W_s$. On remarque que quand $\rho_t$ est proche de zéro, le drift est grand (et positif) et empèche le processus $\rho_t$ de rester proche de zéro.
%
%    
%    C'est pourquoi le résultat qu'on montrera est uniquement le point
%    \textit{i.}.
%\end{remark}}
...
\begin{align*}
  \text{\hyperlink{term2.3}{(2.3)}} %: \dfrac{1}{2} \int_0^t \dfrac{\delta}{(\rho_s^2 + \varepsilon)^{1/2}} + \dfrac{\rho_s^2}{(\rho_s^2 + \varepsilon)^{3/2}}\d s
...
%    $$ \text{\hyperlink{term2.2.2}{(2.2.2)}} \overset{a = \frac{u}{\sqrt{n}}}{=} \tilde C n^{\frac{\delta +1}{2}} \int_{\sqrt{n}}^\infty \dfrac{u^{\delta -1} }{(u^2 + 1)^{3/2}}\d u $$
%
%    Enfin, par le critère de Riemann, cette intégrale converge et en faisant tendre $n$ vers l'infini on obtient que $\text{\hyperlink{term2.2.2}{(2.2.2)}} \xrightarrow[n \to \infty]{} 0$. Ainsi, $\text{\hyperlink{term2.2}{(2.2)}} \xrightarrow[n \to \infty]{} 0$ et on a :
...
%
%\begin{align*}
%  \dfrac{\delta-1}{2}\int_0^t \dfrac{1}{\rho_s} \d s &\overset{\text{\autoref{thm:occupationBessel}}}{=} \int_{\R_+} \dfrac{\delta-1}{2 a} \ell_t^a a^{\delta-1}\d a,\\
%  &\quad\;\;\;=\quad \dfrac{\delta-1}{2} \int_{\R_+} \ell_t^a a^{\delta-2}\d a.
%\end{align*}
%
