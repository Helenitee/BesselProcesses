...
%\end{remark}
%
%{\color{red} \subsection{Caractérisation de Lévy}
%P. Lévy caractérise comme \textbf{l'unique} martingale locale continue $M$ tel que $[M]_t = t$.  }
%\begin{definition}[Brownien Réfléchi]
%  \label{def:BrownienReflechi}
%  \end{definition}
...
%\noindent Dans notre cas, on définit :
%$$\varphi^\varepsilon(W_s) =  \dfrac{W_s-x}{((W_s-x)^2 +\varepsilon)^{1/2}} - \sign(W_s -x).$$
...
% $|\sign(W_s - x)| \leq 1$ et $\left|\dfrac{W_s-x}{((W_s-x)^2 +\varepsilon)^{1/2}}\right| \leq 1$.\\ i.e.
...
%\section{Définition à partir du Mouvement Brownien}
...
%  \begin{equation}
%    Z_t = y + \delta t + 2 \int_0^t \sqrt{Z_s} dB_s, \quad t\geq0.
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\paragraph{Discussion} Maintenant qu'on sait ce que c'est un bessel, notre but est de savoir quelle EDS il satisfait.
%
...
%  \item $\delta \in [0,2]$ $\rightarrow$ we visit an infinite number of times the point 0 $\rightarrow$ problem! we do have a singularity in zero.
