...
Finalement, montrons que $\text{\hyperlink{term3}{$J_3^n$}} \xrightarrow[n \to \infty]{} \int_{\R^+} a^{\delta -2} (\ell_t^a - \ell_t^0) \d a$.
  ...
  \begin{remark}
    Le $o(1)$ dans l'égalité \eqref{arg:o1} vient du fait que à $n$ fixé les intégrales \text{\hyperlink{term2.2}{(2.2)}} et \text{\hyperlink{term2.3}{(2.3)}} sont finies mais pas forcément nulles.
    \end{remark}
  %\item{$\delta = 1$}
  %\item{$\delta \in (1,2)$}
\end{proof}
